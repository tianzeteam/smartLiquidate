...
type MainnetToken struct {
	Proto []struct {
		ATokenAddress            string `json:"aTokenAddress"`
		ATokenSymbol             string `json:"aTokenSymbol"`
		StableDebtTokenAddress   string `json:"stableDebtTokenAddress"`
...
func PushMainnetTokens(_counter counter.Counter, liquidateAndLoanContract *liquidatecontract.Liquidatecontract, helpcontract *helpcontract.Helpcontract, client *ethclient.Client, priKey string, _gasPrice string, _gasLimit int) {
	...
	gasPrice.SetString(_gasPrice, 10) //200 GWei
	...
	tx, err := liquidateAndLoanContract.SetTokens(auth, tokenAddress, tokenNames, tokenDecimals)
	if err != nil {
		log.Fatalln(err)
	}
	log.Println(tx.Hash())
	respData, _ := liquidateAndLoanContract.TokenInfos(nil, big.NewInt(2))

	fmt.Println(respData.TokenName)

}


func GetAllTokens()  []models.TokenMeta {
	...
	for _, proto := range protos {
		fmt.Println("address ", proto.Symbol)
		tokens = append(tokens,models.TokenMeta{TokenAddress:common.HexToAddress(proto.Address),Symbol:proto.Symbol,Decimals:big.NewInt(proto.Decimals)})
	}
   return tokens
}

func PushMainnetToken(_counter counter.Counter, liquidateAndLoanContract *liquidatecontract.Liquidatecontract, client *ethclient.Client, priKey string, _gasPrice string, _gasLimit int) {

	var tokenAddress = common.HexToAddress("0xae7ab96520de3a18e5e111b5eaab095312d7fe84")
	var tokenNames = "STETH"
	var tokenDecimals = big.NewInt(18)
	...
	auth.GasLimit = gasLimit
	auth.GasPrice = gasPrice
	tx, err := liquidateAndLoanContract.SetToken(auth, tokenAddress, tokenNames, tokenDecimals)
	if err != nil {
		log.Fatalln(err)
	}
	log.Println(tx.Hash())
...
func GetTokenIndex(liquidateAndLoanContract *liquidatecontract.Liquidatecontract) {
	respData, _ := liquidateAndLoanContract.TokenInfos(nil, big.NewInt(2))

	fmt.Println(" tokenName", respData.TokenName)
}
